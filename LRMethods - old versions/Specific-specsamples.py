...
train_samples =3000
test_samples = 500
picklefile ='sLRspec' + 'At' + str(test_authors) + 'Atr' + str(train_authors_vec) + 'rep' + str(repeat) + 'ss' + str(sample_size_total) + 'F' + str(n_freq_total) + 'spec' + str(spec_samples_vec)
...
for k_ss in spec_samples_vec:
    for i_ss in sample_size_total:
        for j_ss in n_freq_total:
            ...
            n_freq = j_ss
            spec_samples = k_ss
            train_authors = train_authors_vec[0]
            ...
            for step in tqdm(range(repeat)):
                ...
                for yvalue in np.unique(y):
                    Xset = X[y != yvalue]
                    set = random.sample(range(len(Xset)), 25)
                    Xset = X[set]
                    rv = stats.multivariate_normal(mean=mean_source[yvalue], cov=var_source)
                ...
                trainingsample = random.sample(range(len(labels_ds)),  min(len(labels_ds), train_samples))
