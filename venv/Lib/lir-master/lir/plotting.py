...
def plot_pav(lrs, y, show_scatter=True, savefig=None, show=None, titleplot=None, kw_figure={}):
    """
...
def plot_tippet(LRs, labels, savefig=None, show=None, titleplot=None, kw_figure={}):
    """
    plots the 10log lrs tippett plt
    """
    plt.figure(**kw_figure)
    xplot = np.linspace(np.min(np.log10(LRs)) - 0.1, np.max(np.log10(LRs)) + 0.1, 100)
    LR_0, LR_1 = Xy_to_Xn(LRs, labels)
    perc0 = (sum(i > xplot for i in np.log10(LR_0)) / len(LR_0)) * 100
    perc1 = (sum(i > xplot for i in np.log10(LR_1)) / len(LR_1)) * 100

    plt.plot(xplot, perc1, label='LRs given $\mathregular{H_p}$')
    plt.plot(xplot, perc0, label='LRs given $\mathregular{H_d}$')
...
def plot_score_distribution_and_calibrator_fit(calibrator, scores, y, colorset, savefig=None, show=None, titleplot=None, kw_figure={}):
    """
    ...
    """
    plt.figure(**kw_figure)
    if min(scores)>0 and max(scores)<1:
        x = np.arange(0, 1, .01)
    else:
    ...
    plt.hist(points0, bins=30, alpha=.25, density=True, color=colorset[0])
    plt.hist(points1, bins=30, alpha=.25, density=True, color=colorset[1])
    plt.plot(x, calibrator.p1, color=colorset[1], label = 'Scores given $\mathregular{H_p}$')
    plt.plot(x, calibrator.p0, color=colorset[0], label = 'Scores given $\mathregular{H_d}$')
    plt.legend()
    plt.xlabel('Score')
    plt.ylabel('Density')
    if titleplot is not None:
