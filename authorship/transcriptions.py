...
def read_session(lines, data_name, ground_truth, remove_filler_words=False, expand_contractions=False):
    ...
    if ground_truth:
        ...
        if data_name == 'frida':
            lines_to_words = lines_to_words.replace('Eén', 'één')

            if expand_contractions:
                # only treat the ones I saw
                lines_to_words = (lines_to_words.replace('z\'n', 'zijn').replace('\zo\'n', 'zo een')
                                  .replace('m\'r', 'maar').replace('ok', 'oké').replace('oke', 'oké'))
...
def compile_data(data_name, index_path, ground_truth, info_path=None, remove_filler_words=False,
    ...
    if data_name == 'fisher' and info_path is not None:
        with open(info_path) as f:
            next(f)
            for line in f:
                # each line has the filename, id of speaker, transcriber (LDC or BBN)
                filename, spk_id, trans = line.split('\t', 2)
                to_ids[filename] = [spk_id, trans.replace('\n', '')]
    ...
    for filepath in tqdm(files):
        ...
        if data_name == 'frida':
            ...
            conv_id = (path_str.replace('fe_03_', '').replace('.txt', '').replace('_a', 'a')
                       .replace('-a', 'a').replace('_b', 'b').replace('-b', 'b'))
            file_to_check = (path_str.replace('.txt', '').replace('_a', '-a').replace('_b', '-b'))
        else:
            conv_id = 'error'
        ...
        with open(filepath, "r") as f:
            ...
            if data_name == 'fisher' and info_path is not None:
                if to_ids.get(file_to_check)[1] == 'LDC':
                    continue
