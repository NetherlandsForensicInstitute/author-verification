...
def get_n_most_frequent_words(texts, n):
    """
    Returns n most frequent words from list of texts
    :param texts:
    :param n:
    :return:
    """
...
def get_frequent_table(speakers, wordlist, n):
    ...
    for key, value in sorted(freq.items(), key=lambda x: x[1], reverse=True):
        if (key in freq_words_speak and key in freq_words):
            print('\item {0:}: {1:} x - {2:}\%'.format(key, freq_speak[key],
                                                       '%.2f' % ((freq_speak[key] / totalwords) * 100.0)))
...
def calc_like_common(covar_pop, covar_source, covar_pop_1, covar_source_1, mean_col, n_freq, XXX):
    tot = ((2 * m.pi) ** ((-1 * n_freq)) * (np.linalg.det(covar_pop)) ** (-1) * (np.linalg.det(covar_source)) ** (
        -1) * np.linalg.det((covar_source_1 + 2 * covar_pop_1)) ** (-1 / 2)) * (
              m.exp((-1 / 2) * (np.transpose(np.reshape(XXX[0:n_freq], (n_freq, 1))).dot(covar_source_1).dot(
                  np.reshape(XXX[0:n_freq], (n_freq, 1))) + np.transpose(
                  np.reshape(XXX[n_freq:len(XXX)], (n_freq, 1))).dot(
                  covar_source_1).dot(np.reshape(XXX[n_freq:len(XXX)], (n_freq, 1)))) - (1 / 2) * np.transpose(
                  mean_col).dot(
...
def plot_boxplot_line(X, Values, mean, xaxis, yaxis, boxtitle, color, legenda, on_screen=None, savfig=None,
                      rotation=None, kw_figure={}):
